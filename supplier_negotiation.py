...
class SmartNegotiationAgent:
    def draft_email(self, item_name, current_stock, threshold, supplier_name, units=500, price=None):
        urgency = "CRITICAL" if current_stock < (threshold * 0.2) else "URGENT"
        price_line = f"Target Price: ₹{price:.2f}" if price else "Please provide a bulk-discounted invoice."
        return (f"Subject: [{urgency}] Restock Request for {item_name}\n\n"
                f"Dear {supplier_name},\n\n"
                f"Our AI analysis indicates {item_name} is at {current_stock} units.\n"
                f"To avoid production lag, we require {units} units.\n"
                f"{price_line}\n\n"
...
def simulate_agent_read(item_name, supplier_email):
    ...
    if neg:
        ...
        display_text = f"🤖 AGENT ANALYSIS: Invoice Data Extracted & Validated. | RAW: {raw_reply}"
...
def edit_agent():
    ...
    agent = SmartNegotiationAgent()
    updated_draft = agent.draft_email(neg_detail['item'], neg_detail['stock'], neg_detail['min_limit'], neg_detail['name'], units=new_units, price=new_price)
